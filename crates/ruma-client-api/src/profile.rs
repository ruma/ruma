...
/// Trait implemented by types representing a field in a user's profile.
#[cfg(feature = "unstable-msc4133")]
pub trait ProfileField: Clone {
    /// The type for the value of the field.
    ///
    /// This type should NOT be an `Option<_>`, the APIs already take and return optional values.
...
pub trait StaticProfileField {
    ...
    /// The `ProfileFieldName` representation of this field.
    const NAME: ProfileFieldName;
}

#[cfg(feature = "unstable-msc4133")]
impl<F: StaticProfileField> ProfileField for PhantomData<F> {
    type Value = F::Value;
}
...
impl<'de, F: StaticProfileField> Visitor<'de> for StaticProfileFieldVisitor<F> {
    ...
    fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(formatter, "a map with key `{}` and value", F::NAME)
    }
        ...
        V: MapAccess<'de>,
    {
        ...
        if !found {
            return Err(de::Error::custom("missing field {field}"));
        };
...
impl StaticProfileField for AvatarUrl {
    type Value = Option<OwnedMxcUri>;
    const NAME: ProfileFieldName = ProfileFieldName::AvatarUrl;
}
...
impl StaticProfileField for DisplayName {
    ...
    const NAME: ProfileFieldName = ProfileFieldName::DisplayName;
}
...
#[cfg(feature = "unstable-msc4133")]
impl ProfileField for ProfileFieldName {
    type Value = ProfileFieldValue;
}
...
impl<'de> Visitor<'de> for ProfileFieldValueVisitor {
        ...
        V: MapAccess<'de>,
    {
        let field = if let Some(field) = self.0 {
            ...
            map.next_key()?.ok_or_else(|| {
                de::Error::invalid_length(0, &"should contain at least 1 key-value pair")
            })?
        };
...
mod tests {
    ...
    fn serialize_profile_field_value() {
        // Avatar URL.
        let value = ProfileFieldValue::AvatarUrl(Some(owned_mxc_uri!("mxc://localhost/abcdef")));
        assert_eq!(
            to_json_value(value).unwrap(),
            json!({ "avatar_url": "mxc://localhost/abcdef" })
        );

        let value = ProfileFieldValue::AvatarUrl(None);
        assert_eq!(to_json_value(value).unwrap(), json!({ "avatar_url": null }));

        // Display name.
        let value = ProfileFieldValue::DisplayName(Some("Alice".to_owned()));
        assert_eq!(to_json_value(value).unwrap(), json!({ "displayname": "Alice" }));

        let value = ProfileFieldValue::DisplayName(None);
        assert_eq!(to_json_value(value).unwrap(), json!({ "displayname": null }));
    ...
    fn deserialize_any_profile_field_value() {
        ...
        let json = json!({ "avatar_url": null });
        assert_eq!(
            from_json_value::<ProfileFieldValue>(json).unwrap(),
            ProfileFieldValue::AvatarUrl(None)
        );
        ...
        assert_eq!(
            from_json_value::<ProfileFieldValue>(json).unwrap(),
            ProfileFieldValue::DisplayName(Some("Alice".to_owned()))
        );

        let json = json!({ "displayname": null });
        assert_eq!(
            from_json_value::<ProfileFieldValue>(json).unwrap(),
            ProfileFieldValue::DisplayName(None)
        );
