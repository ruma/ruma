...
mod tests {
    ...
    fn deserialize_response() {
        ...
        assert_eq!(
            response.get_static::<AvatarUrl>().unwrap().unwrap().unwrap(),
            "mxc://localhost/abcdef"
        );
        assert_eq!(response.get_static::<DisplayName>().unwrap().unwrap().unwrap(), "Alice");
        ...
        let response = Response::try_from_http_response(http::Response::new(body)).unwrap();
        assert_eq!(response.get_static::<AvatarUrl>().unwrap().unwrap(), None);
        assert_eq!(response.get_static::<DisplayName>().unwrap().unwrap(), None);
