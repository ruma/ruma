...
pub mod unstable {
    ...
    use ruma_common::{OwnedEventId, OwnedMxcUri, OwnedRoomOrAliasId, OwnedServerName, OwnedTransactionId, OwnedUserId, api::{auth_scheme::AccessToken, request, response}, harm::Harm, metadata};
    ...
    mod serde_server_target {
        ...
        pub(super) fn deserialize<'de, D: Deserializer<'de>>(deserializer: D) -> Result<OwnedServerName, D::Error> {
            let string = <&str>::deserialize(deserializer)?;
    
            if let Some(server_name) = string.strip_prefix("server:") {
                ServerName::parse(server_name).map_err(|_| Error::custom("failed to parse server name"))
            } else {
                Err(Error::custom("not a server"))
            }
        }

        pub(super) fn serialize<S: Serializer>(server_name: &OwnedServerName, serializer: S) -> Result<S::Ok, S::Error> {
