...
- Avoid extra allocations when converting between `OwnedRoomId` /
  ...
  path.
- Add internal `into_raw()` / `from_raw()` helpers to owned IDs generated by
  `IdDst`, and use them to avoid extra allocations when converting between
  `OwnedRoomId` / `OwnedRoomAliasId` and `OwnedRoomOrAliasId` when
