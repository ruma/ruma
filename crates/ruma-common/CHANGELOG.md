...
- Avoid extra allocations when converting between `OwnedRoomId` /
  ...
  path.
- Add `into_raw()` / `from_raw()` to owned IDs generated by `IdDst`, and use
  them to avoid extra allocations when converting between `OwnedRoomId` /
  `OwnedRoomAliasId` and `OwnedRoomOrAliasId` when
