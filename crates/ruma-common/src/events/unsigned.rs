...
use super::{
    ...
    MessageLikeEventContent, OriginalSyncMessageLikeEvent, OriginalSyncStateEvent,
...
pub struct MessageLikeUnsigned<C: MessageLikeEventContent> {
    ...
    pub relations: BundledRelations<OriginalSyncMessageLikeEvent<C>>,
}

impl<C: MessageLikeEventContent> MessageLikeUnsigned<C> {
    /// Create a new `Unsigned` with fields set to `None`.
    pub fn new() -> Self {
        Self { age: None, transaction_id: None, relations: BundledRelations::default() }
    }
}
...
#[derive(Clone, Debug, Deserialize)]
#[serde(
    bound = "OriginalSyncStateEvent<C>: DeserializeOwned, C::PossiblyRedacted: DeserializeOwned"
)]
#[cfg_attr(not(feature = "unstable-exhaustive-types"), non_exhaustive)]
pub struct StateUnsigned<C: StaticStateEventContent> {
    /// The time in milliseconds that has elapsed since the event was sent.
    ///
    /// This field is generated by the local homeserver, and may be incorrect if the local time on
    /// at least one of the two servers is out of sync, which can cause the age to either be
    ...
    pub prev_content: Option<C::PossiblyRedacted>,

    /// [Bundled aggregations] of related child events.
    ///
    /// [Bundled aggregations]: https://spec.matrix.org/latest/client-server-api/#aggregations
    #[serde(rename = "m.relations", default)]
    pub relations: BundledRelations<OriginalSyncStateEvent<C>>,
}

impl<C: StaticStateEventContent> StateUnsigned<C> {
    /// Create a new `Unsigned` with fields set to `None`.
    pub fn new() -> Self {
...
impl<C: StaticStateEventContent> CanBeEmpty for StateUnsigned<C> {
    /// Whether this unsigned data is empty (all fields are `None`).
    ///
    /// This method is used to determine whether to skip serializing the `unsigned` field in room
    /// events. Do not use it to determine whether an incoming `unsigned` field was present - it
