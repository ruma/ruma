...
impl<A: KeyAlgorithm, K: KeyName + ?Sized> KeyId<A, K> {
    ...
    /// Returns the key name of the key ID as a `&str` - the part that comes after the colon.
    pub fn key_name_str(&self) -> &str {
        &self.as_str()[(self.colon_idx() + 1)..]
    }

    /// Returns the typed key name of the key ID - the part that comes after the colon.
    ///
    /// # Example
    ///
    /// ```
