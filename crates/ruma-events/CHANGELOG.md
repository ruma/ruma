...
- The `EventContent` macro never generates the `Redacted*EventContent` and
  `PossiblyRedacted*EventContent` types for state events anymore. The
  `AnyPossiblyRedactedStateEventContent` enum was removed.
- The `RedactContent` implementation generated by the `EventContent` macro for state events always
  sets `type Redacted = Self`, which means that the event content type must now be able to represent
  both original and redacted content.
- The `PossiblyRedactedStateEventContent` trait was removed.
  - The `PossiblyRedacted` associated type was removed from `StaticStateEventContent`
