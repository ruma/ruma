...
- Add `map` methods to event wrappers (`OriginalMessageLikeEvent`,
`OriginalStateEvent`, `StrippedStateEvent`, etc.),
`MessageLikeUnsigned`, and `BundledMessageLikeRelations` to allow transforming content types.
  - Implement `EventContentFromType` for custom event contents generated by `custom_event_content!`.
