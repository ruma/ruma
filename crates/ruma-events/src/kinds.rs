...
impl<C: StaticStateEventContent> StateEventContentChange<C> {
    ...
    pub fn redact(self, rules: &RedactionRules) -> Self
    where
        C: RedactContent<Redacted = C>,
    {
        Self { content: self.content.redact(rules), prev_content: self.prev_content }
    }
}
