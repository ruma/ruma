    ...
    C::Redacted: RedactedMessageLikeEventContent,
{
    fn from(full: MessageLikeEvent<C>) -> Self {
        match full {
            MessageLikeEvent::Original(ev) => Self::Original(ev.into()),
            MessageLikeEvent::Redacted(ev) => Self::Redacted(ev.into()),
        }
    }
}
...
/// list would be sorted lexicographically based on the event's [`OwnedEventId`].
#[derive(Clone, Debug, Event)]
#[cfg_attr(not(ruma_unstable_exhaustive_types), non_exhaustive)]
...
/// A state event content and the corresponding previous content from the unsigned
/// event data, if available.
#[allow(clippy::exhaustive_structs)]
#[derive(Clone, Debug)]
pub struct StateEventContentChange<C: StaticStateEventContent> {
