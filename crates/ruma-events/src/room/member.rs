...
use ruma_common::{
    OwnedMxcUri, OwnedTransactionId, OwnedUserId, ServerSignatures, TransactionId, UserId,
...
use crate::{
    AnyStrippedStateEvent, BundledStateRelations, EventUnsignedData, PrivOwnedStr, RedactContent,
...
impl StrippedRoomMemberEvent {
    ...
    /// should be ignored, and `false` is returned.
    ///
    /// [MSC4293]: https://github.com/matrix-org/matrix-spec-proposals/pull/4293Z
    #[cfg(feature = "unstable-msc4293")]
    pub fn should_redact_events(&self) -> bool {
        self.content.redact_events
            && self.state_key != self.sender
...
pub struct RoomMemberUnsigned {
    ...
    pub redacted_because: Option<Raw<UnsignedRoomRedactionEvent>>,
}
