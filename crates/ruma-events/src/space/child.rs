...
impl SpaceChildEventContent {
    ...
    /// Whether the content is considered "empty", viz. there's no parents anymore (because they've
    /// been cleared/redacted previously).
...
mod tests {
    ...
    fn space_child_empty_serialization() {
        let content = SpaceChildEventContent { via: Some(vec![]), order: None, suggested: false };
