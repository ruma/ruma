...
mod tests {
    ...
    use ruma_events::live_location::BeaconInfoEventContent;
    use ruma_events::location::AssetContent;

    #[test]
    fn beacon_starts_live() {
        let beacon = BeaconInfoEventContent::start("Test Beacon".to_string(), 5000, AssetContent::default());
        assert!(beacon.live);
    }
    #[test]
    fn beacon_stops_live() {
        let mut beacon = BeaconInfoEventContent::start("Test Beacon".to_string(), 5000, AssetContent::default());
        beacon.stop();
        assert!(!beacon.live);
    }
    #[test]
