...
use ruma_events::{
    AnyStateEvent, AnySyncStateEvent, AnyTimelineEvent, room::aliases::RoomAliasesEventContent,
};
...
fn deserialize_aliases_content() {
    ...
    assert_eq!(content.aliases.unwrap(), vec![room_alias_id!("#somewhere:localhost")]);
}
...
fn deserialize_aliases_with_prev_content() {
    ...
    assert_matches!(
        ...
        Ok(AnyStateEvent::RoomAliases(ev))
    );
...
fn deserialize_aliases_sync_with_room_id() {
    ...
    assert_matches!(
        ...
        Ok(AnySyncStateEvent::RoomAliases(ev))
    );
    assert_eq!(ev.content.aliases.unwrap(), vec![room_alias_id!("#somewhere:localhost")]);
    assert_eq!(ev.event_id, "$h29iv0s8:example.com");
    assert_eq!(ev.origin_server_ts, MilliSecondsSinceUnixEpoch(uint!(1)));
...
fn deserialize_full_event_convert_to_sync() {
    ...
    assert_matches!(AnySyncStateEvent::from(full_ev), AnySyncStateEvent::RoomAliases(sync_ev));

    assert_eq!(sync_ev.content.aliases.unwrap(), vec![room_alias_id!("#somewhere:localhost")]);
