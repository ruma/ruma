...
fn deserialize_stripped_state_full_format() {
    use js_int::uint;
    use ruma_common::{event_id, room_id, user_id, MilliSecondsSinceUnixEpoch, RoomVersionId};
    ...
    assert_matches!(
        from_json_value::<AnyStrippedStateEvent>(json).unwrap(),
        AnyStrippedStateEvent::RoomCreate(event)
    );
    assert_eq!(event.content.room_version, RoomVersionId::V11);
    assert_eq!(event.event_id.as_deref(), Some(event_id));
    assert_eq!(event.origin_server_ts, Some(origin_server_ts));
    assert_eq!(event.room_id.as_deref(), Some(room_id));
    assert_eq!(event.sender, user_id);
    assert_eq!(event.state_key.as_ref(), "");
}
