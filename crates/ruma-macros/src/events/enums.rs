...
impl EventKind {
    ...
    pub fn event_enum_variations(self) -> Vec<EventVariation> {
        match self {
            Self::GlobalAccountData | Self::RoomAccountData | Self::ToDevice => {
                vec![EventVariation::None]
            }
            Self::EphemeralRoom => {
                vec![EventVariation::Sync]
            }
            Self::MessageLike | Self::Timeline => {
                vec![EventVariation::None, EventVariation::Sync]
            }
            Self::State => vec![
                EventVariation::None,
                EventVariation::Sync,
            ...
            Self::RoomRedaction | Self::HierarchySpaceChild | Self::Decrypted => vec![],
        }
    }

    /// Get the list of variations for an event type (struct or enum) for this kind.
    pub fn event_variations(self) -> Vec<EventVariation> {
        match self {
            Self::GlobalAccountData
            | Self::RoomAccountData
            | Self::ToDevice
            ...
            Self::Decrypted | Self::Timeline => vec![],
        }
    }

    /// Get the list of variations for an event content type for this kind.
    pub fn event_content_variations(self) -> Vec<EventContentVariation> {
        match self {
            Self::GlobalAccountData
            | Self::RoomAccountData
            | Self::EphemeralRoom
            | Self::ToDevice
            | Self::HierarchySpaceChild => {
                vec![EventContentVariation::Original]
            }
            Self::MessageLike | Self::RoomRedaction => {
                vec![EventContentVariation::Original, EventContentVariation::Redacted]
            }
            Self::State => vec![
...
impl EventField {
    ...
    ///
    /// Returns a `(return_type, is_reference)` tuple.
    pub fn return_type(self, ruma_events: &TokenStream) -> (TokenStream, bool) {
...
impl EventWithBounds {
    pub fn new(
        ...
        let (type_with_generics, impl_generics, where_clause) = match kind {
            ...
            {
                // `MessageLike` and `State` event kinds actually have an extra `RedactContent`
                // bound with a `where` clause on the variations that match enums.
                let redacted_trait =
                    kind.to_content_kind_trait(EventContentTraitVariation::Redacted);
