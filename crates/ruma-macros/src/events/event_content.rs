...
pub fn expand_event_content(
    ...
    let state_key_type = match (event_kind, state_key_types.as_slice()) {
        ...
        (Some(EventKind::State), _) => {
            return Err(syn::Error::new(
                Span::call_site(),
                "multiple state_key_type attributes found, there can only be one",
            ));
        }
        (_, []) => None,
        (_, [ty, ..]) => {
...
fn generate_event_content_impl<'a>(
    ...
    let extra_event_content_impl = match event_kind {
        Some(EventKind::MessageLike) => {
            let unsigned_type = unsigned_type
                .unwrap_or_else(|| quote! { #ruma_common::events::MessageLikeUnsigned });
            quote! {
                #[automatically_derived]
                impl #ruma_common::events::MessageLikeEventContent for #ident {
