...
pub(crate) fn expand_event_content(input: syn::DeriveInput) -> syn::Result<TokenStream> {
    ...
    let possibly_redacted_event_content_impl =
        event_content.expand_possibly_redacted_event_content_impl();
    let redact_content_self_impl = event_content.expand_redact_content_self_impl();
    ...
    Ok(quote! {
        ...
        #possibly_redacted_event_content_impl
        #redact_content_self_impl
        #json_castable_impl
        #event_type_aliases
    })
}
...
impl EventContent {
    ...
    fn expand_redacted_event_content(&self) -> Option<TokenStream> {
        ...
        let redact_content_impl = (!self.kind.should_generate_redact_content_self_impl()).then(|| {
            quote! {
    ...
    /// Generate the `PossiblyRedacted*EventContent` implementation of this struct, if needed.
    fn expand_possibly_redacted_event_content_impl(&self) -> Option<TokenStream> {
        self.kind.should_generate_possibly_redacted_impl().then(|| {
            self.expand_event_content_kind_trait_impl(
    ...
    /// Generate the `RedactContent` implementation of this struct where `Redacted = Self`, if
    /// needed.
    ///
    /// This also generates `PossiblyRedacted*EventContent`, `Redacted*EventContent` and
    /// `From<Redacted*EventContent>` implementations for this type.
    fn expand_redact_content_self_impl(&self) -> Option<TokenStream> {
        ...
        let from_redacted_impl = {
            let redacted_ident = EventContentVariation::Redacted.variation_ident(ident);
            let redacted_field_idents = self
                .fields
                .iter()
                .flatten()
                .filter(|field| field.skip_redaction)
                .filter_map(|field| field.inner.ident.as_ref())
                .collect::<Vec<_>>();
            let from_redacted_field_exprs = self
                .fields
                .iter()
                .flatten()
                .map(|field| {
            ...
            quote! {
                impl From<#redacted_ident> for #ident {
                    fn from(value: #redacted_ident) -> #ident {
                        let #redacted_ident {
            ...
            self.kind
                ...
                .map(|(variation, (type_kind_prefix, event_ident))| {
                    ...
                    let content_ident = if variation.is_redacted()
                        && !matches!(self.kind, EventContentKind::State { .. })
                    {
...
impl EventContentKind {
    ...
    /// Whether we should generate a `RedactContent` implementation for this kind where `Redacted =
    /// Self`.
    fn should_generate_redact_content_self_impl(&self) -> bool {
        // We only generate `RedactContent` implementations with `Redacted = Self` for state events.
        matches!(self, Self::State { has_custom_redacted, .. } if !*has_custom_redacted)
    }

    /// Whether we should generate a `PossiblyRedacted*EventContent` trait implementation for this
    /// kind.
    fn should_generate_possibly_redacted_impl(&self) -> bool {
        // We only generate possibly redacted content implementations for state events.
