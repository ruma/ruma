...
pub struct EventContentAttrs {
    ...
    /// Whether the `Redacted*EventContent` type and/or `RedactContent` implementation are
    /// implemented manually rather than generated by this macro.
...
impl EventContentAttrs {
    ...
    /// Set that the `Redacted*EventContent` type and/or `RedactContent` implementation are
    /// implemented manually rather than generated by this macro.
    ///
    /// Returns an error if it is already set.
            ...
            EventContentKindAttr::Single(kind) => match kind {
                ...
                CommonEventKind::State => {
                    ...
                    let unsigned_type = unsigned_type.unwrap_or_else(|| {
                        parse_quote! {
                            #ruma_events::StateUnsigned<Self>
                        }
                    });

                    EventContentKind::State { state_key_type, unsigned_type, has_custom_redacted }
                }
