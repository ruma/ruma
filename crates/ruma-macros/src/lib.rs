...
///   `StateKind`. It also generates the `RedactContent` implementation which applies the redaction
///   algorithm according to the Matrix specification.
///
...
/// the event content without the `relates_to` field. This macro can generate a
...
/// dynamically. It must be associated with a field that has the `type_fragment` attribute that will
/// store the end of the event type.
///
/// ### `kind = Kind`
///
...
/// ### `alias = "m.event_type"`
///
/// An alternate `type` for the event, used during deserialization. It can usually be used for
