...
fn valid_membership_change(
    ...
    Ok(match target_membership {
        ...
        // TODO: Does this need to be gated
        #[cfg(feature = "unstable-pre-spec")]
        MembershipState::Knock if room_version.allow_knocking => {
            ...
            if join_rules != JoinRule::Knock {
                ...
                // 3. If the `sender`'s current membership is not `ban`, `invite`, or `join`,
                // allow.
                // 4. Otherwise, reject.
                if sender != target_user {
                    warn!(
...
mod tests {
    ...
    fn test_restricted_join_rule() {
        ...
        *events.get_mut(&event_id("IJR")).unwrap() = to_pdu_event(
        ...
        member.join_authorised_via_users_server = Some(alice());
        events.insert(
            event_id("new"),
            to_pdu_event(
                "new",
