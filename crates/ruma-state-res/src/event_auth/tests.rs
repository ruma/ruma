...
use crate::{
    ...
    test_utils::{
        ...
        to_pdu_event, PduEvent, INITIAL_EVENTS,
    },
    RoomVersion,
};
...
fn missing_room_create_in_state() {
    ...
    assert!(!auth_check(&RoomVersion::V6, incoming_event, None::<PduEvent>, fetch_state).unwrap());
}
...
fn room_aliases_no_state_key() {
    ...
    assert!(!auth_check(&RoomVersion::V3, &incoming_event, None::<PduEvent>, fetch_state).unwrap());

    // `m.room.aliases` is not checked since v6.
    assert!(auth_check(&RoomVersion::V9, &incoming_event, None::<PduEvent>, fetch_state).unwrap());
}
