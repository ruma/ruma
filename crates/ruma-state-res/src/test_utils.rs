...
use ruma_common::{RoomId, room_id, room_version_rules::RoomIdFormatVersion};
...
pub fn default_room_id(format: &RoomIdFormatVersion) -> RoomId {
    match format {
        RoomIdFormatVersion::V1 => room_id!("!room:matrix.local"),
        // The default ID of the `m.room.create` event.
        RoomIdFormatVersion::V2 => room_id!("!room-create"),
