...
fn test_resolve(paths: &[&str]) -> Snapshots {
    ...
    if iteratively_resolved_state != batched_resolved_state {
        let diff = unified_diff(
            Algorithm::default(),
            &to_json_string_pretty(&iteratively_resolved_state)
                .expect("should be able to serialize iteratively resolved state"),
            &to_json_string_pretty(&batched_resolved_state)
                .expect("should be able to serialize batched resolved state"),
            3,
            Some(("iterative", "batched")),
        );

        panic!("iterative and batched results should match; but they differ:\n{diff}");
    }
    ...
    II: IntoIterator<IntoIter = I> + Clone,
{
    ...
    let pdus_by_id: HashMap<OwnedEventId, Pdu> = HashMap::from_iter(
        pdus.clone().into_iter().map(|pdu| (pdu.event_id().to_owned(), pdu.to_owned())),
    );
    ...
    'outer: while let Some(event_id) = stack.pop() {
        ...
        for prev_event in current_pdu.prev_events() {
            ...
            for pdu in state_at_event
                .values()
                .map(|event_id| pdus_by_id.get(event_id).expect("every pdu should be available"))
            {
                auth_chain_sets.push(auth_events_dfs(&pdus_by_id, pdu)?);
            }
        ...
        let state_before_event =
            resolve(auth_rules, &states_before_event, auth_chain_sets.clone(), |x| {
        ...
        if let Some(prev_events) = forward_prev_events_graph.get(&event_id) {
            stack.extend(prev_events.clone().into_iter().cloned());
        } else {
