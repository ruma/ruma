...
mod access_token {
    ...
    fn extract_authentication() -> Result<(), Box<dyn std::error::Error + 'static>> {
        ...
        assert_matches!(authentication, Authentication::AccessToken(token) if token == "header_access_token");
...
mod query_access_token {
    ...
    fn extract_authentication() -> Result<(), Box<dyn std::error::Error + 'static>> {
        ...
        assert_matches!(authentication, Authentication::AccessToken(token) if token == "query_access_token");
...
mod query_only_access_token {
    ...
    fn test_access_token() -> Result<(), Box<dyn std::error::Error + 'static>> {
        ...
        assert_matches!(authentication, Authentication::AccessToken(token) if token == "query_only_access_token");
...
mod server_signatures {
    ...
    fn extract_authentication() -> Result<(), Box<dyn std::error::Error + 'static>> {
        ...
        assert_matches!(
            authentication,
            Authentication::ServerSignatures(headers) if headers[0].origin == "theirserver.tld"
